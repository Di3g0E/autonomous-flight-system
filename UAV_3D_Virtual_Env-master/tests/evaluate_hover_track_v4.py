#!/usr/bin/env python
"""
Multi-checkpoint evaluator for hover-track v4 (moving target).
...
tier and writes a per-checkpoint summary + global comparison table.

Tiers vary target speed AND initial offset/velocity:
  slow:   speed=0.10, offset=0.3m, vel=0.10
  medium: speed=0.25, offset=0.6m, vel=0.25
  fast:   speed=0.40, offset=1.0m, vel=0.40

Default checkpoints: every 50k step + best_model.

Usage:
    python tests/evaluate_hover_track_v4.py
    python tests/evaluate_hover_track_v4.py --checkpoints 100000 200000 300000
    python tests/evaluate_hover_track_v4.py --episodes-per-tier 10 --duration 20
    python tests/evaluate_hover_track_v4.py --model-dir ./models/hover_track_v3_1
"""
...
# ── Defaults ──────────────────────────────────────────────────────────
CKPT_DIR = './models/hover_track_v4/checkpoints'
BEST_MODEL = './models/hover_track_v4/best_model.zip'
DURATION_S = 20
EPISODES_PER_TIER = 10

DEFAULT_STEPS = [50_000, 150_000, 250_000, 350_000, 500_000, 750_000]

TIERS = {
    'slow':   {'speed': 0.10, 'offset': 0.3, 'vel': 0.10, 'ang': 0.05},
    'medium': {'speed': 0.25, 'offset': 0.6, 'vel': 0.25, 'ang': 0.10},
    'fast':   {'speed': 0.40, 'offset': 1.0, 'vel': 0.40, 'ang': 0.15},
}


def parse_args():
    p = argparse.ArgumentParser(
        description="Evaluate hover-track v4 checkpoints (moving target)")
    p.add_argument('--model-dir', type=str, default='./models/hover_track_v4',
                   help="Model directory with checkpoints/ and best_model.zip")
    ...
    p.add_argument('--output-dir', type=str,
                   default='./experiments/hover_track_v4')
    return p.parse_args()


def discover_checkpoints(model_dir, requested_steps):
...
class EvalV4App(ShowBase):
    def __init__(self, args):
        ...
        # Environment — moving target, v3.1 reward
        self.env = Panda3DQuadrotorEnv(
            ...
            target_mode='moving',
            target_range=3.0,
            target_speed=0.10,  # overridden per tier
        ...
        self.env.min_start_distance = 0.0  # allow target near drone

        self.checkpoints = discover_checkpoints(
            args.model_dir, args.checkpoints)
    ...
    def _run_single_episode(self, model, tier_name, tier_cfg, ep_idx):
        """Run one episode with the given tier configuration."""
        # Configure tier
        self.env.target_speed = tier_cfg['speed']
        ...
        # Reposition drone above target + offset
        ...
        state[0] = target_pos[0] + dx
        state[2] = target_pos[1] + dy
        state[4] = target_pos[2] + self.env.hover_height
        ...
        self.env.base_env.state = state
        self.env._update_visualization()

        self.graphicsEngine.renderFrame()
        self.env._capture_camera_images(force_capture=True)
        obs = self.env._build_observation(state.astype(np.float32))

        # Warm-up steps (let physics settle)
    ...
    def _evaluate_checkpoint(self, step_num, model_path):
        ...
        for tier_name in ('slow', 'medium', 'fast'):
            cfg = TIERS[tier_name]
            print(f"  -- {tier_name.upper()} "
                  f"(spd={cfg['speed']} off={cfg['offset']}m "
                  f"vel={cfg['vel']}) --")
    ...
    def run_evaluation(self):
        ...
        output = {
            ...
            'lemniscate_scale': self.args.lemniscate_scale,
    ...
    def _print_comparison(self, all_results):
        print(f"\n{'='*105}")
        print("  CHECKPOINT COMPARISON — HOVER-TRACK v4 (moving target)")
        print(f"{'='*105}")
    ...
    def _generate_plots(self, all_results):
        ...
        for ax, (key, title) in zip(axes.flat, metrics):
            ...
            ax.set_title(title)
            ax.grid(alpha=0.3)
        fig.suptitle('v4 Checkpoint Comparison — Global Metrics (moving target)',
                     fontsize=14)
        fig.tight_layout()
        fig.savefig(str(self.output_dir / 'checkpoint_global.png'), dpi=150)
        plt.close(fig)

        # ── 2. Per-tier survival + reward + visibility ──
        fig, axes = plt.subplots(1, 3, figsize=(15, 5))
        tier_colors = {'slow': '#4CAF50', 'medium': '#2196F3',
                       'fast': '#FF5722'}
        tier_metrics = [
        ...
        for ax, (key, title) in zip(axes, tier_metrics):
            ...
            ax.legend()
            ax.grid(alpha=0.3)
        fig.suptitle('v4 Checkpoint Comparison — Per Tier', fontsize=14)
        fig.tight_layout()
