...
import numpy as np
from src.simulation.quaternion_euler_utility import euler_quat, quat_euler, deriv_quat, quat_rot_mat
...
class quad(gym.Env):
    ...
    A Gymnasium-compatible environment for quadrotor control and reinforcement learning.
    
    Observation Space:
        Box(13,) containing [x, vx, y, vy, z, vz, q0, q1, q2, q3, w_x, w_y, w_z]
        - Position (x, y, z) and velocities (vx, vy, vz) in inertial frame
        - Quaternion orientation (q0, q1, q2, q3)
        - Angular velocities (w_x, w_y, w_z) in body frame
    
    Action Space:
        Box(4,) containing normalized motor forces in range [-1, 1]
    """
    
    metadata = {'render_modes': ['human'], 'render_fps': 30}

    def __init__(self, t_step, n, euler=0, direct_control=0, T=1, render_mode=None):
        ...
        # Observation space bounds based on bounding box conditions
        obs_low = np.array([
            -BB_POS, -BB_VEL,  # x, vx
            -BB_POS, -BB_VEL,  # y, vy
            -BB_POS, -BB_VEL,  # z, vz
            -1, -1, -1, -1,    # quaternion (normalized)
            -BB_VEL, -BB_VEL, -BB_VEL  # angular velocities
        ], dtype=np.float32)
    ...
    def reset(self, seed=None, options=None):
        ...
        Args:
            ...
            options: Optional dict that may contain 'det_state' for deterministic initial state
                if euler flag is on:
        ...
        Returns:
            observation: Initial observation (numpy array of shape (13,))
        ...
        # Check if deterministic state is provided in options
        det_state = None
        if options is not None and 'det_state' in options:
            det_state = options['det_state']
        ...
        # Warm-up steps with zero control
        for i in range(self.T):
            self.action = self.zero_control
            self.action_hist.append(self.action)
            # Temporarily set done to False to allow step execution
            temp_done = self.done
            self.done = False
            obs, reward, terminated, truncated, info = self.step(self.action)
            self.done = temp_done
        
        # Return initial observation and info dict
        observation = self.previous_state.astype(np.float32)
        info = {
            'solved': self.solved,
            'angular_position': self.ang if hasattr(self, 'ang') else None
        }
    ...
    def step(self, action):
        ...
        Args:
            action: Action to be applied on the system (numpy array of shape (4,))
        
        Returns:
            observation: Current state observation (numpy array of shape (13,))
            reward: Reward obtained from this step
            terminated: Whether the episode has terminated (boundary violation or success)
            truncated: Whether the episode was truncated (time limit reached)
            info: Dictionary with additional information
        """
        ...
        # Distinguish between terminated and truncated
        # terminated: episode ends due to boundary violation or success
        ...
        if self.done:
            if self.i >= self.n:
                ...
                # Boundary violation or success
                terminated = True
        
        # Additional info
        info = {
            'solved': self.solved,
            'angular_position': self.ang,
    ...
    def render(self):
        ...
        Currently returns None. Can be extended to provide visualization
        using matplotlib or other rendering libraries.
        ...
        if self.render_mode == 'human':
            ...
            # Could use the existing plotter class or create new visualization
            pass
        return None
