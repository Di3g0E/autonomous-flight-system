...
import numpy as np
import sys
from src.envs.quadrotor_env import quad, sensor
from src.simulation.quaternion_euler_utility import deriv_quat
...
class quad_position():
    ...
    def __init__(self, render, quad_model, prop_models, EPISODE_STEPS, REAL_CTRL, ERROR_AQS_EPISODES, ERROR_PATH, HOVER):
        ...
        try:
            import os
            weights_path = os.path.join(os.path.dirname(__file__), '..', '..', 'weights', 'ppo', 'PPO_continuous_solved_drone.pth')
            self.policy.load_state_dict(torch.load(weights_path, map_location=device))
    ...
    def drone_position_task(self, task):
        if task.frame == 0 or self.env.done:
            ...
            if self.HOVER:
                ...
                states, info = self.env.reset(options={'det_state': in_state})
            else:
                states, info = self.env.reset()
            # Initialize action as zero control for first frame
            action = self.env.zero_control
            ...
            for i, w_i in enumerate(self.env.w):
                # Ensure w_i is a scalar
                if isinstance(w_i, (list, tuple, np.ndarray)):
                    w_i_val = float(w_i[0]) if len(w_i) > 0 else 0.0
                else:
                    w_i_val = float(w_i)
                self.a[i] += (w_i_val*time_int_step )*180/np.pi/10
