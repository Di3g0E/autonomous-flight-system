...
def world_setup(env, render, mydir):
    # Add assets folder to the model search path so Panda3D can find 
    # the /textures/ folder referenced in the .egg models
    getModelPath().appendDirectory(mydir + "/assets")
    
    env.disableMouse()
    
    # Load the environment model.
    env.scene = env.loader.loadModel(mydir + "/assets/models/city.egg")
    env.scene.reparentTo(env.render)
    env.scene.setScale(1, 1, 1)
    env.scene.setPos(0, 0, 0)
    
    # Load the skybox
    env.skybox = env.loader.loadModel(mydir + "/assets/models/skybox.egg")
...
def quad_setup(env, render, mydir):
    ...
    env.quad_model = env.loader.loadModel(mydir + '/assets/models/quad.egg')
    env.quad_model.reparentTo(env.render)
    env.prop_1 = env.loader.loadModel(mydir + '/assets/models/prop.egg')
    env.prop_1.setPos(-0.26,0,0)
    env.prop_1.reparentTo(env.quad_model)
    env.prop_2 = env.loader.loadModel(mydir + '/assets/models/prop.egg')
    env.prop_2.setPos(0,0.26,0)
    env.prop_2.reparentTo(env.quad_model)
    env.prop_3 = env.loader.loadModel(mydir + '/assets/models/prop.egg')
    env.prop_3.setPos(0.26,0,0)
    env.prop_3.reparentTo(env.quad_model)
    env.prop_4 = env.loader.loadModel(mydir + '/assets/models/prop.egg')
    ...
    env.checker = env.loader.loadModel(mydir + '/assets/models/checkerboard.egg')
