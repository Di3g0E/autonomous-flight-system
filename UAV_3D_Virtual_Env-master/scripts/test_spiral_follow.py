#!/usr/bin/env python
"""
Test the spiral_follow PPO model in ISOLATION (no target, no v10.4).

Loads the trained spiral PPO model and runs it inside its exact training
environment (SpiralFollowEnv on top of Panda3DQuadrotorEnv with no
camera and no target). Records a bird's-eye video with telemetry overlay
showing drone trajectory vs spiral reference.

Purpose: diagnostic. If the spiral works well here it means the model
itself is fine; any failure in the integrated full-flight demo is then
attributable to the integration (handoff, OOD position, velocities at
transition), not the spiral model itself.

Usage:
    # Default (omega_scale=1.0 to match inference in SpiralSearchController)
    python scripts/test_spiral_follow.py --no-display

    # Match late training conditions (slower spiral)
    python scripts/test_spiral_follow.py --no-display --omega-scale 0.7

    # Multiple seeds for robustness
    python scripts/test_spiral_follow.py --no-display --n-episodes 3
"""

import argparse
import math
import os
import sys
import time
from pathlib import Path

import cv2
import numpy as np

try:
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')
    sys.stderr.reconfigure(encoding='utf-8', errors='replace')
except (AttributeError, OSError):
    pass

project_root = str(Path(__file__).parent.parent)
if project_root not in sys.path:
    sys.path.insert(0, project_root)

import torch  # noqa: F401  must precede SB3 to use miniconda torch
from panda3d.core import Filename, loadPrcFile, loadPrcFileData

loadPrcFile(Filename.fromOsSpecific(
    os.path.join(project_root, 'config', 'conf.prc')))

from direct.showbase.ShowBase import ShowBase
from stable_baselines3 import PPO
from stable_baselines3.common.vec_env import DummyVecEnv, VecNormalize
from stable_baselines3.common.monitor import Monitor

from src.simulation.world_setup import world_setup, quad_setup
from src.simulation.camera_control import camera_control
from src.vision.img_2_cv import opencv_camera
from src.envs.panda3d_quadrotor_env import Panda3DQuadrotorEnv
from src.envs.spiral_follow_env import SpiralFollowEnv


def parse_args():
    p = argparse.ArgumentParser(
        description="Test spiral_follow PPO model in its training env (no v10.4)")
    p.add_argument('--model', default='./models/spiral_follow/best_model.zip')
    p.add_argument('--n-episodes', type=int, default=1)
    p.add_argument('--max-ep-steps', type=int, default=2000,
                   help="2000 = 20 s, the training default.")
    p.add_argument('--omega-scale', type=float, default=1.0,
                   help="Curriculum multiplier on omega. 1.0 = full spiral "
                        "(matches SpiralSearchController). 0.3-0.7 = the regime "
                        "the model saw during most of training (Phase A).")
    p.add_argument('--omega', type=float, default=1.8)
    p.add_argument('--r-growth', type=float, default=0.12)
    p.add_argument('--hover-height', type=float, default=1.39)
    p.add_argument('--vision-radius', type=float, default=0.5)
    p.add_argument('--seed-base', type=int, default=2000)
    p.add_argument('--no-display', action='store_true')
    p.add_argument('--output-path', type=str, default='')
    p.add_argument('--fps', type=int, default=20)
    p.add_argument('--frame-step', type=int, default=1,
                   help="capture every Nth sim step. Default 1 = every step "
                        "(catches early termination). Use 5 for real-time playback.")
    p.add_argument('--win-size', type=str, default='1920x1080')
    p.add_argument('--panel-size', type=str, default='1280x720')
    p.add_argument('--deterministic', type=int, default=1,
                   help="1 = use mean of policy (deterministic). 0 = sample "
                        "stochastically (matches training). Try both if needed.")
    p.add_argument('--verbose', action='store_true',
                   help="Print state, action, reward at every step.")
    p.add_argument('--use-vecnormalize', action='store_true', default=True,
                   help="Wrap env in Monitor+DummyVecEnv+VecNormalize and load "
                        "vecnormalize.pkl. Reproduces the exact training stack.")
    return p.parse_args()


# ──────────────────────────────────────────────────────────────────────
# App
# ──────────────────────────────────────────────────────────────────────

class SpiralTestApp(ShowBase):
    def __init__(self, args):
        ShowBase.__init__(self)
        self.args = args
        mydir = Filename.fromOsSpecific(
            os.path.abspath(project_root)).getFullpath()

        print("Loading 3D world...")
        world_setup(self, self.render, mydir)
        quad_setup(self, self.render, mydir)
        camera_control(self, self.render)
        self.taskMgr.remove('Camera Movement')

        # Bird's-eye external camera — wide enough to see the whole spiral
        # at r ≈ 2.5 m
        self.cam.reparentTo(self.render)
        self.cam.setPos(0, -2, 14)
        self.cam.lookAt(0, 0, 1.4)

        self.ext_camera = opencv_camera(self, 'ext_cam', 1)
        self.ext_camera.cam.reparentTo(self.render)
        self.ext_camera.cam.setPos(0, -2, 14)
        self.ext_camera.cam.lookAt(0, 0, 1.4)
        self.ext_camera.buffer.setActive(1)

        # Build env EXACTLY as in training:
        #   Panda3DQuadrotorEnv(use_camera=False, use_target=False, ...) +
        #   SpiralFollowEnv wrapper.
        # init_pos_range=0.1, init_vel_range=0.05 are the training values.
        print(f"Creating env (no camera, no target, max_steps={args.max_ep_steps})...")
        self.base_env = Panda3DQuadrotorEnv(
            panda3d_app=self,
            quad_model=self.quad_model,
            render_node=self.render,
            use_camera=False,
            use_depth=False,
            use_target=False,
            enable_collisions=False,
            n=args.max_ep_steps,
            t_step=0.01,
            direct_control=1,
            filming_mode=True,
            use_new_reward=True,
            constrained_init=True,
            init_pos_range=0.1,
            init_vel_range=0.05,
            init_ang_range=0.03,
        )

        self.spiral_env = SpiralFollowEnv(
            self.base_env,
            omega=args.omega,
            r_growth=args.r_growth,
            hover_height=args.hover_height,
            vision_radius=args.vision_radius,
        )
        # Apply the curriculum multiplier (training reached 1.0 by end)
        self.spiral_env.omega_scale = float(args.omega_scale)

        # Reproduce the EXACT training stack: Monitor → DummyVecEnv → VecNormalize.
        # The previous version skipped these wrappers and the model was loaded
        # without env, which prevented automatic action clipping and produced
        # immediate BB violations (empty MP4s of 258 bytes).
        if args.use_vecnormalize:
            mon = Monitor(self.spiral_env)
            self.vec_env = DummyVecEnv([lambda: mon])
            vp = Path(args.model).parent / 'vecnormalize.pkl'
            if vp.exists():
                self.vec_env = VecNormalize.load(str(vp), self.vec_env)
                self.vec_env.training = False
                self.vec_env.norm_reward = False
                print(f"Loaded VecNormalize stats: {vp}")
            else:
                self.vec_env = VecNormalize(
                    self.vec_env, norm_obs=False,
                    norm_reward=False, gamma=0.99)
                print(f"WARNING: vecnormalize.pkl not found, using fresh VecNormalize")
            # Pass env into PPO.load so policy learns the action_space and
            # automatically clips actions to [-1, 1] during predict().
            self.model = PPO.load(args.model, env=self.vec_env, device='auto')
        else:
            self.vec_env = None
            self.model = PPO.load(args.model, device='auto')
        print(f"Loaded spiral PPO model: {args.model}")
        print(f"omega_scale: {args.omega_scale}  deterministic={bool(args.deterministic)}")


# ──────────────────────────────────────────────────────────────────────
# Drawing helpers
# ──────────────────────────────────────────────────────────────────────

def world_to_panel(x_w, y_w, panel_w, panel_h, view_extent=3.5):
    """Project a world XY coordinate onto the bird's-eye panel.

    The bird's-eye camera looks straight down at origin from above; this
    is a simple orthographic projection. view_extent is the world
    half-width visible in each panel axis (in metres).
    """
    cx, cy = panel_w // 2, panel_h // 2
    px = int(cx + x_w / view_extent * cx)
    py = int(cy - y_w / view_extent * cy)  # invert Y for screen coords
    return px, py


def draw_overlay(panel, info, panel_w, panel_h, drone_pos, ref_pos, history):
    """Draw spiral reference, drone trail and telemetry on bird's-eye panel."""
    font = cv2.FONT_HERSHEY_SIMPLEX

    # Title bar
    cv2.putText(panel, "Bird's-eye + spiral reference", (10, 28),
                font, 0.65, (255, 255, 255), 2)

    # Trail of drone positions (cyan)
    if len(history) >= 2:
        for i in range(1, len(history)):
            x1, y1 = world_to_panel(*history[i - 1], panel_w, panel_h)
            x2, y2 = world_to_panel(*history[i], panel_w, panel_h)
            cv2.line(panel, (x1, y1), (x2, y2), (0, 255, 255), 2)

    # Drone (green dot)
    dpx, dpy = world_to_panel(drone_pos[0], drone_pos[1], panel_w, panel_h)
    cv2.circle(panel, (dpx, dpy), 8, (0, 255, 0), -1)
    cv2.circle(panel, (dpx, dpy), 8, (255, 255, 255), 1)
    cv2.putText(panel, "drone", (dpx + 12, dpy - 6),
                font, 0.45, (0, 255, 0), 1)

    # Reference (red dot)
    rpx, rpy = world_to_panel(ref_pos[0], ref_pos[1], panel_w, panel_h)
    cv2.circle(panel, (rpx, rpy), 7, (255, 0, 0), -1)
    cv2.circle(panel, (rpx, rpy), 7, (255, 255, 255), 1)
    cv2.putText(panel, "ref", (rpx + 12, rpy - 6),
                font, 0.45, (255, 0, 0), 1)

    # Connection line drone-ref to visualise position error
    cv2.line(panel, (dpx, dpy), (rpx, rpy), (200, 200, 200), 1)

    # Telemetry block (top-right)
    y = 60
    rows = [
        ('Step',     f"{info.get('Step', 0):>4}"),
        ('Time',     f"{info.get('Step', 0) * 0.01:>5.2f} s"),
        ('Drone XY', f"({drone_pos[0]:+.2f}, {drone_pos[1]:+.2f})"),
        ('Drone Z',  f"{drone_pos[2]:.3f} m"),
        ('Ref r',    f"{info.get('SpiralR', 0):.3f} m"),
        ('Pos err',  f"{info.get('PosErr', 0):.3f} m"),
        ('Alt err',  f"{abs(drone_pos[2] - 1.39):.3f} m"),
    ]
    x_text = panel_w - 280
    cv2.rectangle(panel, (x_text - 10, y - 30),
                  (panel_w - 5, y + len(rows) * 26),
                  (0, 0, 0), -1)
    cv2.putText(panel, "-- Telemetry --", (x_text, y - 8),
                font, 0.5, (255, 255, 255), 1)
    for k, v in rows:
        cv2.putText(panel, f"{k:>9}: {v}", (x_text, y), font, 0.5, (0, 255, 0), 1)
        y += 24


def get_bird_image(ext_camera, panel_w, panel_h):
    ok, rgba = ext_camera.get_image()
    if not ok:
        return np.zeros((panel_h, panel_w, 3), dtype=np.uint8)
    rgb = cv2.cvtColor(rgba, cv2.COLOR_RGBA2RGB)
    return cv2.resize(rgb, (panel_w, panel_h), interpolation=cv2.INTER_CUBIC)


# ──────────────────────────────────────────────────────────────────────
# Episode runner
# ──────────────────────────────────────────────────────────────────────

def run_episode(app, args, seed, output_path, panel_w, panel_h):
    # Reset via the vec_env if we wrapped, else via raw spiral_env.
    if app.vec_env is not None:
        obs = app.vec_env.reset()  # shape (1, 18)
    else:
        obs, _ = app.spiral_env.reset(seed=seed)
    app.taskMgr.step()

    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    writer = cv2.VideoWriter(str(output_path), fourcc, args.fps,
                              (panel_w, panel_h))
    if not writer.isOpened():
        raise RuntimeError(f"Could not open VideoWriter at {output_path}")

    history = []
    pos_errors = []
    alt_errors = []
    max_r = 0.0
    cum_reward = 0.0
    step = 0
    deterministic = bool(args.deterministic)
    last_term_reason = ''

    while step < args.max_ep_steps:
        act, _ = app.model.predict(obs, deterministic=deterministic)
        if app.vec_env is not None:
            # VecEnv API returns (obs, rewards, dones, infos)
            obs, rewards, dones, infos = app.vec_env.step(act)
            r = float(rewards[0])
            done = bool(dones[0])
            info = infos[0] if infos else {}
            term = info.get('TimeLimit.truncated') is False and done
            trunc = info.get('TimeLimit.truncated', False)
        else:
            single_act = act[0] if act.ndim > 1 else act
            obs, r, term, trunc, info = app.spiral_env.step(single_act)
            done = term or trunc
        app.taskMgr.step()
        cum_reward += float(r)
        step += 1

        # Read drone state and reference from spiral env internals
        st = app.base_env.base_env.state  # Panda3DQuadrotorEnv → underlying physics QuadrotorEnv
        drone_pos = (float(st[0]), float(st[2]), float(st[4]))
        ref_pos = (float(app.spiral_env._ref_x),
                   float(app.spiral_env._ref_y))
        spiral_r = float(app.spiral_env._ref_r)
        pos_err = math.sqrt((drone_pos[0] - ref_pos[0]) ** 2 +
                            (drone_pos[1] - ref_pos[1]) ** 2)
        alt_err = abs(drone_pos[2] - args.hover_height)

        pos_errors.append(pos_err)
        alt_errors.append(alt_err)
        history.append((drone_pos[0], drone_pos[1]))
        max_r = max(max_r, spiral_r)
        if len(history) > 600:
            history = history[-600:]

        if args.verbose:
            act_disp = act[0] if hasattr(act, 'shape') and act.ndim > 1 else act
            print(f"  step={step:>4}  drone=({drone_pos[0]:+.2f},"
                  f"{drone_pos[1]:+.2f},{drone_pos[2]:.2f})  "
                  f"ref=({ref_pos[0]:+.2f},{ref_pos[1]:+.2f})  r={spiral_r:.2f}  "
                  f"pos_err={pos_err:.3f}  alt_err={alt_err:.3f}  "
                  f"act=[{act_disp[0]:+.2f},{act_disp[1]:+.2f},"
                  f"{act_disp[2]:+.2f},{act_disp[3]:+.2f}]  r={r:+.3f}  "
                  f"term={term} trunc={trunc}")

        if step % args.frame_step == 0:
            bird = get_bird_image(app.ext_camera, panel_w, panel_h)
            info_overlay = {'Step': step, 'SpiralR': spiral_r,
                             'PosErr': pos_err}
            draw_overlay(bird, info_overlay, panel_w, panel_h,
                         drone_pos, ref_pos, history)
            writer.write(cv2.cvtColor(bird, cv2.COLOR_RGB2BGR))

        if app.vec_env is not None:
            if done:
                last_term_reason = (f"VecEnv done at step {step} "
                                    f"(term={term}, trunc={trunc})")
                break
        else:
            if term or trunc:
                last_term_reason = f"term={term} trunc={trunc} at step {step}"
                break

    writer.release()
    return {
        'seed': seed,
        'steps': step,
        'mean_pos_error': float(np.mean(pos_errors)) if pos_errors else 0.0,
        'p95_pos_error': float(np.percentile(pos_errors, 95)) if pos_errors else 0.0,
        'mean_alt_error': float(np.mean(alt_errors)) if alt_errors else 0.0,
        'max_r_reached': max_r,
        'cum_reward': cum_reward,
        'video_path': str(output_path),
        'term_reason': last_term_reason,
    }


# ──────────────────────────────────────────────────────────────────────
# main
# ──────────────────────────────────────────────────────────────────────

def main():
    args = parse_args()

    win_w, win_h = (int(x) for x in args.win_size.lower().replace(' ', 'x').split('x'))
    panel_w, panel_h = (int(x) for x in args.panel_size.lower().split('x'))

    if args.no_display:
        loadPrcFileData('', 'window-type offscreen')
        loadPrcFileData('', f'win-size {win_w} {win_h}')
        loadPrcFileData('', 'undecorated true')
    else:
        loadPrcFileData('', f'win-size {win_w} {win_h}')

    app = SpiralTestApp(args)

    outdir = Path('./models/spiral_follow/recordings')
    outdir.mkdir(parents=True, exist_ok=True)

    print("\n" + "=" * 70)
    print(f"  SPIRAL FOLLOW STANDALONE TEST  (omega_scale={args.omega_scale})")
    print("=" * 70)

    results = []
    for i in range(args.n_episodes):
        seed = args.seed_base + i
        if args.output_path and args.n_episodes == 1:
            video_path = Path(args.output_path)
            video_path.parent.mkdir(parents=True, exist_ok=True)
        else:
            scale_tag = f"{args.omega_scale:.2f}".replace('.', '')
            video_path = outdir / f"spiral_test_seed{seed}_omega{scale_tag}.mp4"
        print(f"\n--- Episode {i + 1}/{args.n_episodes} (seed={seed}) ---")
        t0 = time.time()
        result = run_episode(app, args, seed, video_path, panel_w, panel_h)
        elapsed = time.time() - t0
        result['wall_clock_s'] = elapsed
        results.append(result)
        print(f"  Steps:           {result['steps']}/{args.max_ep_steps}")
        print(f"  Mean pos error:  {result['mean_pos_error']:.3f} m")
        print(f"  P95 pos error:   {result['p95_pos_error']:.3f} m")
        print(f"  Mean alt error:  {result['mean_alt_error']:.3f} m")
        print(f"  Max r reached:   {result['max_r_reached']:.3f} m")
        print(f"  Cum reward:      {result['cum_reward']:.1f}")
        if result['term_reason']:
            print(f"  Termination:     {result['term_reason']}")
        print(f"  Video:           {result['video_path']}")

    # Aggregate
    print("\n" + "=" * 70)
    print("  AGGREGATE  ({} episodes)".format(args.n_episodes))
    print("=" * 70)
    if args.n_episodes >= 1:
        for k in ('mean_pos_error', 'p95_pos_error', 'mean_alt_error',
                  'max_r_reached', 'cum_reward'):
            vals = [r[k] for r in results]
            print(f"  {k:>20}: mean={np.mean(vals):.3f}  "
                  f"std={np.std(vals):.3f}")


if __name__ == "__main__":
    main()
